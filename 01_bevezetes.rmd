...
A következőkben egy példát mutatunk be a szövegbányászat gyakorlati alkalmazására vonatkozóan, amely során Ady Endre és Petőfi Sándor verseinek szóhasználatát hasonlítjuk össze, az eredményekhez pedig szemléletes ábrázolást is készítünk. A jelen példában alkalmazott eljárásokat a későbbi fejezetekben részletesen is kifejtjük itt csupán szemléltetni kívánjuk velük, hogy milyen jellegű elemzéseket sajátíthat majd el az olvasó a könyv segítségével.

 Az R számos kiegészítő csomaggal rendelkezik, amelyek hasznos funkcióit úgy érhetünk el, ha telepítjük az őket tartalmazó csomagot. Ezt az `install.packages` paranccsal tehetjük meg, ha már korábban egyszer elvégeztük, akkor nem szükséges egy csomaggal megismételni.
...
```{r}
ady <- readtext("data/ady.txt")
petofi <- readtext("data/petofi.txt")
```

Ahhoz, hogy adatokat használni tudjunk azokon először mindig úgynevezett tisztítási folyamatokat kell elvégeznünk, ezek segítségével tudjuk előkészíteni a szövegeket, ha nem végezzük el őket, vagy elvégezzük, de kellő alaposság nélkül akkor az eredményeink félrevezetőek lesznek.
...
ady_tok <- tokens(ady_corpus, what = "word", remove_symbols = TRUE, remove_numbers = TRUE, remove_punct = TRUE) %>% 
  ...
  tokens_remove (c("is", "ha", "te", "föl", "kis", "előtt", "oli", "ő")) %>%
  tokens_wordstem()

adydfm <- dfm(ady_tok, tolower = TRUE, remove = stopwords("hungarian"), stem = TRUE, remove_punct = TRUE, remove_symbols = TRUE, remove_numbers = TRUE)
...
petofidfm <- dfm(petofi_corpus, tolower = TRUE, remove = stopwords("hungarian"), stem = TRUE, remove_punct = TRUE, remove_symbols = TRUE, remove_numbers = TRUE)
...
dfm_trim(petofidfm, min_termfreq = 50, min_docfreq = 2)

dfm_trim(petofidfm, max_termfreq = 5, max_docfreq = 2)
