...
![A szövegbányászat egyes módszerei](figures/01_modszerek.jpg)
...
```{r, eval=FALSE}
# hunminer placeholder

ady <- data_ady_corpus
petofi <- data_petofi_corpus
```


```{r}
ady <- readtext("data/ady.txt", encoding = "UTF8")
petofi <- readtext("data/petofi_corpus/corpus_files/*.txt", encoding = "UTF8")

petofi <- data.frame(text = paste(petofi$text, collapse = " "),
                     doc_id = "petofi")
...
Első lépésként corpus formátumba helyezzük az adatainkat, erre csupán azért van szükség mert a későbbiekben ebből a formátumból tudjuk majd őket, token formátumba helyezni. 
...
ady_tok <- tokens(ady_corpus, what = "word", remove_symbols = TRUE, remove_numbers = TRUE, remove_punct = TRUE) %>% 
  ...
  tokens_remove (c("is", "ha", "te", "föl", "kis", "előtt", "oli", "ő", "a")) %>%
  tokens_wordstem()

petofi_tok <- tokens(petofi_corpus, what = "word", remove_symbols = TRUE, remove_numbers = TRUE, remove_punct = TRUE) %>% 
  tokens_tolower() %>% 
  tokens_remove(stopwords("hungarian")) %>% 
  tokens_remove (c("is", "ha", "te", "föl", "kis", "előtt", "oli", "ő", "a")) %>%
  tokens_wordstem()
...
```{r}
adydfm <- dfm_trim(petofidfm, min_termfreq = 5)

petofidfm <- dfm_trim(adydfm, min_termfreq = 5)
