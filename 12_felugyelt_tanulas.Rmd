...
Jelen fejezetben két modell fajtát mutatunk be egyazon korpuszon az SVM-t (support vector machine) és a Naive Bayes-t. Mindkettő a fentiekben leírt klasszifikációs feladatot végzi el, viszont eltérő módon működnek. Az SVM minden dokumentumunkat egy vektorként reprezentálja, ami annyit jelent, hogy hozzájuk rendel egy számsort, amely egy közös térben betöltött pozíciójukat reprezentálja. Az SVM lényegében egy vonalat (hyperplane) húz meg ebben a közös térben úgy, hogy az elválassza egymástól a tanítóhalmaz különféle módon felcímkézett dokumentumait. Innentől kezdve a modellnek nincs más dolga, mint megállapítani azon dokumentumok vektor értékeit, amelyek nem voltak a teszt halmaz részei, attól függően, hogy az SVM által meghúzott vonal mely felére esnek ezek is besorolhatóvá válnak a teszthalmazunk kategóriáiba. Ezzel szemben a Naive Bayes a dokumentumok egyes szavaihoz rendel valószínűségi értékeket. A felcímkézett teszt halmazunk szavaihoz egy valószínűségi értéket rendel annak függvényében, hogy az adott szó adott kategóriába tartozó dokumentumokban hányszor jelenik meg, az adott kategória dokumentumainak teljes szószámához képest. Miután a teszt halmazunk dokumentumaiban minden szóhoz hozzárendeltük ezeket a valószínűségi értékeket nincs más dolgunk, mint a teszt halmazunkon kívül eső dokumentumokban felkeresni ugyanezen szavakat, a hozzájuk rendelt valószínűségi értékeket aggregálni és ez alapján minden dokumentumhoz tudunk rendelni több valószínűségi értéket is, amelyek megadják, hogy mekkora eséllyel tartozik a dokumentum a teszt halmazunk egyes kategóriáiba.
...
```{r include = TRUE}
dfmat_training <- dfm_subset(dfmtnol, id_numeric %in% id_train)
dfmat_test <- dfm_subset(dfmtnol, !id_numeric %in% id_train)
``` 

Azáltal pedig, hogy kialakítottuk a két alhalmazt el is végeztük az utolsó előkészítési folyamatot is, az így előkészített adatokon tudjuk majd futtatni a következőkben, mind a Naive Bayes és az SVM modellünket.

### Naive Bayes

Először létrehozzuk a modellünket, amely számára meghatározzuk, hogy a label változóhoz kell egy predikciót adnia, majd ezt alakalmazzuk az adatainkra.
...
```{r include = TRUE}
tmod_svm <- textmodel_svm(dfmat_training, y = dfmat_training$label)
``` 
...
```{r include = TRUE}
actual_class_svm <- dfmat_matched$label
predicted_class_svm <- predict(tmod_svm, newdata = dfmat_matched)
``` 
